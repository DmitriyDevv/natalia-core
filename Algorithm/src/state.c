...
static TransportAckStatus ack_status_from_result(ActionResult result) {
    switch (result) {
    case ACTION_OK:
        return TRANSPORT_ACK_OK;

    case ACTION_ERR_CONTENT:
        return TRANSPORT_ACK_ERR_CONTENT;

    case ACTION_ALARM:
    case ACTION_ERR_OTHER:
    default:
        return TRANSPORT_ACK_ERR_OTHER;
    }
}

static void reject_command(const SystemEvent* event) {
    (void)action_send_ack_status(event, TRANSPORT_ACK_ERR_MODE);
}
